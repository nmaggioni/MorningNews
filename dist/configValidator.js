...
function createMissingKeyError(key) {
    return new Error(`Missing "${key}" config key`);
}
function createBadKeyTypeError(key, expected) {
    return new Error(`Bad "${key}" config key type, expected ${expected}`);
}
function validateExternalConfig(externalConfig) {
    ...
    for (const feed of externalConfig.feeds) {
        ...
        for (const entry of feed.schema) {
            if (typeof (entry) !== "string" || entry !== "title" /* TITLE */ && entry !== "description" /* DESCRIPTION */) {
                return createBadKeyTypeError(`feeds[${feedsCounter}].schema[]`, `${"title" /* TITLE */} or ${"description" /* DESCRIPTION */}`);
            }
        }
        if (!feed.count) {
...
exports.validateExternalConfig = validateExternalConfig;
